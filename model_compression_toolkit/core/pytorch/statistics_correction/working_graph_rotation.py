...
def is_reshape(n):
    return n.type in [torch.reshape, torch.Tensor.view, torch.permute, torch.Tensor.contiguous]
...
def check_block(graph, first_node, second_node):
    ...
    if is_norm(next_first_node_wo_add[0]):
        ...
        prev_second_node_wo_add = [psn for psn in prev_second_node if not is_add(psn)]

...
def get_rotationable_chains(graph):
    ...
    for idx, node in enumerate(nodes):
        ...
        chain = [node]
        # visited.add(node)
        chain_visited = {node}  # Temporary visited set
        ...
        while True:
            ...
            if node_outputs is None or node_outputs in visited or node_outputs in chain_visited:
                break

            chain.append(node_outputs)
            # visited.add(node_outputs)
            chain_visited.add(node_outputs)
        ...
        if valid:
            chains.append(chain)
            visited.update(chain_visited)  # Mark the entire chain as visited only now
...
def rotate_graph(graph: Graph):
    ...
    for chain in rotationable_chains:
        ...
        # output_nodes = []
        # norm_nodes = []
        rotationable_subchains = []
        current_valid_nodes = []
        current_input_nodes = []
        current_output_nodes = []
        current_norm_nodes = []
        for idx in range(len(chain) - 1):
            first_node = chain[idx]
            ...
            if len(rotationable_block) == NUM_BRANCHES:  # should be 2?
                if not current_valid_nodes:
                    current_valid_nodes.append(first_node)
                ...
                current_norm_nodes.append(norm_node[0])
                    ...
                    # input_nodes = []
                    # output_nodes = []
                # Save current subchain if non-empty
                if len(current_valid_nodes) >= 2:
                    rotationable_subchains.append({
                        "valid_nodes": current_valid_nodes,
                        "input_nodes": current_input_nodes,
                        "output_nodes": current_output_nodes,
                        "norm_nodes": current_norm_nodes
                    })

                # Reset for next subchain
                current_valid_nodes = []
                current_input_nodes = []
                current_output_nodes = []
                current_norm_nodes = []

        # Edge case: if the chain ended with a valid block
        if len(current_valid_nodes) >= 2:
            rotationable_subchains.append({
                "valid_nodes": current_valid_nodes,
                "input_nodes": current_input_nodes,
        ...
        for sub_chain in rotationable_subchains:
            valid_nodes = sub_chain["valid_nodes"]
            input_nodes = sub_chain["input_nodes"]
            ...
            end_chain_node = graph.get_next_nodes(valid_nodes[-1])
            if len(end_chain_node) == 1:
                end_chain_node = end_chain_node[0]
                ...
                matmul_head_node = BaseNode(name=matmul_head_name,
                                            framework_attr={'in_features': R1.shape[0], 'out_features': R1.shape[0],
                                                            BIAS: False},
                                            input_shape=end_chain_node.output_shape,
                                            output_shape=end_chain_node.output_shape,
                                            weights={'weight': R1.T},
                                            layer_class=nn.Linear)

                matmul_head_node.candidates_quantization_cfg = copy.deepcopy(input_nodes[0].candidates_quantization_cfg)
                matmul_head_node.prior_info = copy.deepcopy(input_nodes[0].prior_info)
                insert_node_after_node(graph, matmul_head_node, end_chain_node)

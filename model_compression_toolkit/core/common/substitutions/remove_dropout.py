...
def remove_dropout_node(graph: Graph,
                         node: BaseNode) -> Graph:
    prev_dropout_nodes = graph.get_prev_nodes(node)

    # Ensure there is exactly one predecessor; otherwise, do nothing.
    if len(prev_dropout_nodes) != 1:
        # We do not expect to get here.
        Logger.error(f"Dropout node {node} have {len(prev_dropout_nodes)} inputs, while expected to have one. Skipping remove dropout substitution.") # pragma: no cover
        return graph  # pragma: no cover

    graph_outputs = graph.get_outputs()
    for i, g_out in enumerate(graph_outputs):
        if g_out.node == node:
            graph_outputs[i] = OutTensor(node=prev_dropout_nodes[0], node_out_index=g_out.node_out_index)

    # Reconnect the output edges of the identity node to its predecessor,
    # effectively bypassing the identity node.
    graph.reconnect_out_edges(current_node=node, new_node=prev_dropout_nodes[0])
    # Remove the edge from the predecessor to the identity node.
    graph.remove_edge(prev_dropout_nodes[0], node)
